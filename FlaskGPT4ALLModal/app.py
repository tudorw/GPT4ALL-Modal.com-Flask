...
def ask_question():
    ...
    while exchange_count < num_runs:
        ...
        if exchange_count > 1 :
            ...
            arc_profile_prompt= ('The following is an instruction, your goal is to return to me just one scene number in numeric format. use this yaml format text which lists acts and scenes within acts "'+ arc +'" to find the scene number in this yaml file, in node "Scene: #:" that best matches this text " ' + conversation_history_joined +' "  take your best guess, it does not matter if you are wrong, if you fail, return the number 1, take care to return a number and only a number, a computer will read the number and will error if it is returned with text, in case of failure, return 1')
            ...
            #get example text
            scene_instructions_prompt= ('use' + arc +' to return stage directions for scene' + arc_profile)
            scene_instructions = get_answer_from_model(model_choice_2,  scene_instructions_prompt)
        ...
        writers_instructions = arc_profile + " is the story arc type chosen for this story"
        ...
        antagonist_yaml_profile_prompt= ('Take the following YAML and return a short but emotionally charged plain english description of the individual and their traits.' + content)
        ...
        with open('static/data/cast/ZebraZero/profile.yml', 'r') as file:
            content = file.read()
        content = content.replace('\n', ' ')
        protagonist_profile_prompt= ('Take the following YAML and return a short but emotionally charged plain english description of the individual and their traits.' + content)
        ...
        conversation_prompt2= (conversation_prompt + answer_1 + ':is the story so far. Here is a profile of the character in the story that I want you to act as; ' +  protagonist_profile + '. So what would you say and do next? Please reply with only story content and dialogue.')
